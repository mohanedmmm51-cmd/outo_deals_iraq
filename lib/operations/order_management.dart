  ...
  Future<void> _accept(
    ...
    final agreed = await showDialog<bool>(
      ...
      builder: (dialogContext) => AlertDialog(
        title: const Text('الموافقة على السعر والعمولة'),
        content: Text(
          'سعر البيع للزبون: ${opMoney(price)} د.ع\n'
          'عمولة التطبيق ضمن السعر: ${opMoney(commission)} د.ع\n'
          'الصافي للمحل بعد العمولة: ${opMoney(price - commission)} د.ع\n\n'
          'بموافقتك تلتزم بهذا السعر. تستحق العمولة بعد تنفيذ البيع وتدخل بالتسوية الأسبوعية.',
        ),
        actions: [
          TextButton(
          ...
          FilledButton(
            ...
            child: const Text('أوافق على السعر والعمولة'),
          ),
        ],
      ),
    );
    ...
    try {
        ...
        if (current == null ||
            !(current['status'] == 'new' ||
                (['accepted', 'on_the_way'].contains(current['status']) &&
                    current['acceptedPrice'] == null)) ||
            current['completed'] == true ||
            current['price'] != price ||
            current['commission'] != commission ||
            !opDate(current['expiresAt']).isAfter(DateTime.now())) {
          throw StateError('الطلب تغير أو انتهت صلاحيته. حدّث الصفحة');
        }
        tx.update(ref, {
          'status': 'accepted',
          'acceptedAt': FieldValue.serverTimestamp(),
          'acceptedPrice': price,
          'acceptedCommission': commission,
        });
      });
      ...
      if (context.mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text(
              'تعذر قبول الطلب. تأكد من الاتصال وصلاحية الطلب وحاول مجدداً',
            ),
          ),
        );
      }
      ...
      builder: (context, shopSnap) {
        ...
        if (shop == null)
          return const Scaffold(
            body: Center(child: Text('سجل دخول المحل أولاً')),
          );
        return Scaffold(
            ...
            child: StreamBuilder<QuerySnapshot<Map<String, dynamic>>>(
              ...
              builder: (context, snap) {
                ...
                final docs = snap.data!.docs.toList()
                  ..sort(
                    (a, b) =>
                        opDate(b.data()['createdAt'])
                            .compareTo(opDate(a.data()['createdAt'])),
                  );
                  ...
                  itemBuilder: (_, i) {
                    ...
                    return Card(
                      child: ListTile(
                        ...
                        subtitle: Text(
                          '${d.id}\n${orderStatusLabel(status)} • ${opMoney((x['price'] as num?)?.toInt() ?? 0)} د.ع\nعمولة التطبيق ضمن السعر: ${opMoney((x['commission'] as num?)?.toInt() ?? 0)} د.ع',
                        ),
                        isThreeLine: true,
                        trailing:
                            status == 'new' ||
...
class _OrdersManagementPageState extends State<OrdersManagementPage> {
  ...
  List<QueryDocumentSnapshot<Map<String, dynamic>>> _filter(
    ...
    return docs.where((d) {
      ...
      return '${d.id} ${x['title']} ${x['shopName']} ${x['detail']}'
          .toLowerCase()
          .contains(q);
    }).toList();
  }
  ...
  Future<void> _exportCsv(
    ...
    await SharePlus.instance.share(
      ShareParams(
        ...
        text: 'تقرير طلبات Auto Deals Iraq',
      ),
    );
  }
  ...
  Future<void> _exportPdf(
    ...
    final total = docs.fold<int>(
      0,
      (value, d) => value + ((d.data()['price'] as num?)?.toInt() ?? 0),
    );
    final commission = docs.fold<int>(
      0,
      (value, d) => value + ((d.data()['commission'] as num?)?.toInt() ?? 0),
    );
    pdf.addPage(
      pw.MultiPage(
        ...
        build: (_) => [
          pw.Text(
            'Auto Deals Iraq Orders Report',
            style: pw.TextStyle(fontSize: 20, fontWeight: pw.FontWeight.bold),
          ),
          pw.SizedBox(height: 8),
          pw.Text(
            'Orders: ${docs.length}  Sales: $total IQD  Commission: $commission IQD',
          ),
              ...
              children: [
                ...
                Row(
                  children: [
                    ...
                    Expanded(
                      child: FilledButton.icon(
                        onPressed: docs.isEmpty ? null : () => _exportPdf(docs),
      ...
      body: Directionality(
        ...
        child: StreamBuilder<QuerySnapshot<Map<String, dynamic>>>(
          ...
          builder: (context, snap) {
            ...
            final commissions = completed.fold<int>(
              0,
              (value, d) =>
                  value + ((d.data()['commission'] as num?)?.toInt() ?? 0),
            );
            ...
            final topShops = shops.entries.toList()
              ..sort((a, b) => b.value.compareTo(a.value));
            return ListView(
