...
class _OrderActionsPageState extends State<OrderActionsPage> {
  ...
  Future<void> _setStatus(String status) async {
    ...
    await ref.set({'status': status, 'statusUpdatedAt': FieldValue.serverTimestamp()}, SetOptions(merge: true));
    await AuditLogService.record(action: 'order_status_$status', targetType: 'order', targetId: widget.orderCode);
  }
  ...
  Future<void> _addNote() async {
    ...
    await FirebaseFirestore.instance.collection('orders').doc(widget.orderCode).collection('notes').add({
      'text': text,
  ...
  Future<void> _share(Map<String, dynamic> data) async {
    final text = 'طلب Auto Deals Iraq\n'
        ...
        'الحالة: ${orderStatusLabel('${data['status'] ?? (data['completed'] == true ? 'completed' : 'new')}')}';
      ...
      body: Directionality(
        ...
        child: StreamBuilder<DocumentSnapshot<Map<String, dynamic>>>(
          ...
          builder: (context, snap) {
            ...
            final expired = expires.millisecondsSinceEpoch > 0 && DateTime.now().isAfter(expires) && status != 'completed' && status != 'cancelled';
            ...
            return ListView(
              ...
              children: [
                ...
                FilledButton.icon(onPressed: () => _share(data), icon: const Icon(Icons.share), label: const Text('مشاركة الطلب')),
                const SizedBox(height: 8),
                if (!terminal)
                  OutlinedButton.icon(onPressed: () => _setStatus('on_the_way'), icon: const Icon(Icons.directions_car), label: const Text('أنا بالطريق للمحل')),
                if (!terminal)
                  TextButton.icon(onPressed: _cancel, icon: const Icon(Icons.cancel), label: const Text('إلغاء الطلب')),
                const SizedBox(height: 12),
                const Text('ملاحظات الطلب', style: TextStyle(fontSize: 19, fontWeight: FontWeight.bold)),
                const SizedBox(height: 8),
                Row(children: [
                  Expanded(child: TextField(controller: note, decoration: const InputDecoration(hintText: 'اكتب ملاحظة', border: OutlineInputBorder()))),
                ...
                StreamBuilder<QuerySnapshot<Map<String, dynamic>>>(
                  ...
                  builder: (context, notesSnap) {
                    ...
                    return Column(children: notesSnap.data!.docs.map((d) => Card(child: ListTile(leading: const Icon(Icons.note_alt_outlined), title: Text('${d.data()['text'] ?? ''}')))).toList());
                  },
                ),
              ],
            );
...
class ShopOrdersPage extends StatelessWidget {
  ...
  Future<void> _accept(DocumentReference<Map<String, dynamic>> ref) async {
    await ref.set({'status': 'accepted', 'acceptedAt': FieldValue.serverTimestamp()}, SetOptions(merge: true));
        ...
        return Scaffold(
          ...
          body: Directionality(
            ...
            child: StreamBuilder<QuerySnapshot<Map<String, dynamic>>>(
              ...
              builder: (context, snap) {
                if (!snap.hasData) return const Center(child: CircularProgressIndicator());
                final docs = snap.data!.docs.toList()..sort((a, b) => opDate(b.data()['createdAt']).compareTo(opDate(a.data()['createdAt'])));
                return ListView.builder(
                  ...
                  itemBuilder: (_, i) {
                    ...
                    return Card(child: ListTile(
                      ...
                      isThreeLine: true,
                      trailing: status == 'new' ? FilledButton(onPressed: () => _accept(d.reference), child: const Text('قبول')) : null,
                    ));
                  },
                );
              },
...
class _OrdersManagementPageState extends State<OrdersManagementPage> {
  ...
  List<QueryDocumentSnapshot<Map<String, dynamic>>> _filter(List<QueryDocumentSnapshot<Map<String, dynamic>>> docs) {
    ...
    return docs.where((d) {
      ...
      final haystack = '${d.id} ${x['title']} ${x['shopName']} ${x['detail']}'.toLowerCase();
      return haystack.contains(q);
    }).toList();
  }

  Future<void> _pickFrom() async {
    final d = await showDatePicker(context: context, firstDate: DateTime(2025), lastDate: DateTime.now().add(const Duration(days: 365)), initialDate: from ?? DateTime.now());
    if (d != null) setState(() => from = d);
  }

  Future<void> _pickTo() async {
    final d = await showDatePicker(context: context, firstDate: DateTime(2025), lastDate: DateTime.now().add(const Duration(days: 365)), initialDate: to ?? DateTime.now());
  ...
  Future<void> _exportCsv(List<QueryDocumentSnapshot<Map<String, dynamic>>> docs) async {
    ...
    for (final d in docs) {
      ...
      var s = '${x['status'] ?? ''}';
      if (s.isEmpty) s = x['completed'] == true ? 'completed' : 'new';
      String esc(dynamic v) => '"${'$v'.replaceAll('"', '""')}"';
      lines.add([esc(d.id), esc(x['title']), esc(x['shopName']), esc(s), x['price'] ?? 0, x['commission'] ?? 0, esc(opDate(x['createdAt']).toIso8601String())].join(','));
    }
    final dir = await getTemporaryDirectory();
    final file = File('${dir.path}/auto_deals_orders.csv');
    await file.writeAsString(lines.join('\n'));
    await Share.shareXFiles([XFile(file.path)], text: 'تقرير طلبات Auto Deals Iraq');
  }

  Future<void> _exportPdf(List<QueryDocumentSnapshot<Map<String, dynamic>>> docs) async {
    final pdf = pw.Document();
    final total = docs.fold<int>(0, (sum, d) => sum + ((d.data()['price'] as num?)?.toInt() ?? 0));
    final commission = docs.fold<int>(0, (sum, d) => sum + ((d.data()['commission'] as num?)?.toInt() ?? 0));
        ...
        child: StreamBuilder<QuerySnapshot<Map<String, dynamic>>>(
          ...
          builder: (context, snap) {
            ...
            return ListView(
              padding: const EdgeInsets.all(12),
              children: [
                TextField(controller: search, onChanged: (_) => setState(() {}), decoration: const InputDecoration(prefixIcon: Icon(Icons.search), hintText: 'ابحث بالكود أو المحل أو المنتج', border: OutlineInputBorder())),
                const SizedBox(height: 8),
                ...
                ...docs.map((d) {
                  ...
                  var s = '${x['status'] ?? ''}';
                  if (s.isEmpty) s = x['completed'] == true ? 'completed' : 'new';
                  return Card(child: ListTile(title: Text('${x['title'] ?? ''}'), subtitle: Text('${d.id}\n${x['shopName'] ?? ''} • ${orderStatusLabel(s)} • ${opMoney((x['price'] as num?)?.toInt() ?? 0)} د.ع'), isThreeLine: true));
                }),
              ],
            );
          },
...
class AnalyticsPage extends StatelessWidget {
      ...
      body: Directionality(
        ...
        child: StreamBuilder<QuerySnapshot<Map<String, dynamic>>>(
          ...
          builder: (context, snap) {
            ...
            final completed = docs.where((d) => d.data()['completed'] == true || d.data()['status'] == 'completed').toList();
            final sales = completed.fold<int>(0, (sum, d) => sum + ((d.data()['price'] as num?)?.toInt() ?? 0));
            final commissions = completed.fold<int>(0, (sum, d) => sum + ((d.data()['commission'] as num?)?.toInt() ?? 0));
            ...
            final topShops = shops.entries.where((e) => e.key.isNotEmpty).toList()..sort((a, b) => b.value.compareTo(a.value));
            return ListView(padding: const EdgeInsets.all(16), children: [
              Row(children: [Expanded(child: _stat('مبيعات', '${opMoney(sales)} د.ع', Icons.payments)), const SizedBox(width: 8), Expanded(child: _stat('عمولات', '${opMoney(commissions)} د.ع', Icons.account_balance_wallet))]),
              const SizedBox(height: 8),
  ...
  Widget _stat(String title, String value, IconData icon) => Card(color: operationsYellow, child: Padding(padding: const EdgeInsets.all(14), child: Column(children: [Icon(icon, size: 32), Text(title), Text(value, style: const TextStyle(fontSize: 18, fontWeight: FontWeight.bold))])));
}
...
class _PricingManagementPageState extends State<PricingManagementPage> {
  ...
  Future<void> _save() async {
    ...
    await AuditLogService.record(action: 'price_rule_update', targetType: 'price_rule', targetId: id, details: '$c/$k=$v');
    category.clear(); keyName.clear(); value.clear();
  }
  ...
  Widget build(BuildContext context) => Scaffold(
        ...
        body: Directionality(
          ...
          child: StreamBuilder<QuerySnapshot<Map<String, dynamic>>>(
            ...
            builder: (context, snap) => ListView(padding: const EdgeInsets.all(16), children: [
              ...
              const SizedBox(height: 12),
              ...?snap.data?.docs.map((d) { final x = d.data(); return Card(child: ListTile(title: Text('${x['category']} • ${x['key']}'), subtitle: Text('${opMoney((x['value'] as num?)?.toInt() ?? 0)} د.ع'), trailing: IconButton(onPressed: () => d.reference.delete(), icon: const Icon(Icons.delete_outline)))); }),
            ]),
          ),
        ),
      );
            ...
            body: Directionality(
              ...
              child: StreamBuilder<QuerySnapshot<Map<String, dynamic>>>(
                ...
                builder: (context, snap) => ListView(padding: const EdgeInsets.all(16), children: [
                  ...
                  const SizedBox(height: 12),
                  ...?snap.data?.docs.map((d) { final x = d.data(); final q = (x['quantity'] as num?)?.toInt() ?? 0; return Card(child: ListTile(title: Text('${x['name'] ?? ''}'), subtitle: Text(q > 0 ? 'متوفر • الكمية $q' : 'غير متوفر'), trailing: Switch(value: x['available'] == true, onChanged: (v) => d.reference.set({'available': v}, SetOptions(merge: true)))); }),
                ]),
              ),
            ),
          );
...
class _BranchManagementPageState extends State<BranchManagementPage> {
  ...
  Future<void> _add(ShopProfile shop) async {
    ...
    name.clear(); address.clear(); phone.clear();
  }
            ...
            body: Directionality(
              ...
              child: StreamBuilder<QuerySnapshot<Map<String, dynamic>>>(
                ...
                builder: (context, snap) => ListView(padding: const EdgeInsets.all(16), children: [
                  ...
                  const SizedBox(height: 12),
                  ...?snap.data?.docs.map((d) { final x = d.data(); return Card(child: ListTile(title: Text('${x['name'] ?? ''}'), subtitle: Text('${x['address'] ?? ''}\n${x['phone'] ?? ''}'), isThreeLine: true, trailing: Switch(value: x['active'] != false, onChanged: (v) => d.reference.set({'active': v}, SetOptions(merge: true)))); }),
                ]),
              ),
            ),
          );
  ...
  Widget build(BuildContext context) => Scaffold(
        ...
        body: Directionality(
          ...
          child: StreamBuilder<QuerySnapshot<Map<String, dynamic>>>(
            ...
            builder: (context, snap) {
              if (!snap.hasData) return const Center(child: CircularProgressIndicator());
              return ListView(padding: const EdgeInsets.all(12), children: snap.data!.docs.map((d) { final x = d.data(); return Card(child: ListTile(title: Text('${x['action'] ?? ''}', style: const TextStyle(fontWeight: FontWeight.bold)), subtitle: Text('${x['targetType'] ?? ''}: ${x['targetId'] ?? ''}\n${x['actorEmail'] ?? ''} • ${x['details'] ?? ''}'), isThreeLine: true)); }).toList());
            },
          ),
        ),
      );
