...
class OrderStore {
  ...
  static Future<AppOrder> create({
    ...
    await db.runTransaction((tx) async {
      ...
      if (!shopSnap.exists || shopData == null || shopData['approved'] != true) {
  ...
  static Future<AppOrder?> confirm(
    ...
    final result = await db.runTransaction<AppOrder?>((tx) async {
      ...
      if (current.productId.trim().isNotEmpty &&
        ...
        final inventoryRef = InventoryService.itemRef(shopId, current.productId);
        final inventorySnap = await tx.get(inventoryRef);
        final inventory = inventorySnap.data();
        final quantity = (inventory?['quantity'] as num?)?.toInt() ?? 0;
        if (!inventorySnap.exists || inventory?['available'] != true || quantity <= 0) {
          throw StateError('المنتج صار نافد. لا يمكن تنفيذ الطلب أو تسجيل العمولة');
        }
    ...
    return ListView(
      ...
      children: [
          ...
          child: Padding(
            ...
            child: Column(
              ...
              children: [
                ...
                const Text('بعد إنشاء الكود ما يقدر المحل يغيّر سعر هذا الطلب.'),
              ],
            ),
          ),
        ),
        ...
        FilledButton.icon(
          ...
          label: Text(busy ? 'جاري تثبيت الطلب...' : 'إنشاء الكود وتثبيت السعر'),
        ),
        if (error != null)
          Padding(
            ...
            child: Padding(
              ...
              child: Column(
                ...
                children: [
                  ...
                  const Text('صلاحية الكود: 24 ساعة • السعر لا يتغير خلال الصلاحية'),
                ],
              ),
            ),
          ),
                            ...
                            child: ListTile(
                              onTap: () async {
                                await Navigator.push(
                                  ...
                                  MaterialPageRoute(
                                    builder: (_) =>
                                        OrderActionsPage(orderCode: o.code),
                                  ),
                                );
                                _load();
                              },
                              leading: CircleAvatar(
                                backgroundColor:
                                    status == 'completed' ? Colors.green : orderYellow,
                                child: Icon(
                                  status == 'completed'
                                      ? Icons.check
                                      : Icons.receipt_long,
